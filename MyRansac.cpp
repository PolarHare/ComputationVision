...
pair<M, vector<bool>> ransac(vector<X> points, int minPointsCount, float threshold,
    ...
    vector<bool> curInliers(points.size(), false);
    for (int iter = 0; iter < itersCount; iter++) {
        ...
        int curInliersCount = 0;
        for (int i = 0; i < points.size(); i++) {
            float rank = rankingFoo(model, points[i]);
            if (rank <= threshold) {
                curInliers[i] = true;
                curInliersCount++;
            } else {
        ...
        if (curInliersCount > bestInliersCount) {
            bestInliers = curInliers;
            bestInliersCount = curInliersCount;
        }
    }
    vector<X> inliers;
    for (int i = 0; i < points.size(); i++) {
    ...
    return pair<M, vector<bool>>(createModelFoo(inliers), bestInliers);
}
...
int main(int argc, char **argv) {
    if (argc != 2) {
        cout << "Usage: MyRansac <folderName>" << endl
                << "Where folderName - folder with 1.jpg and 2.jpg images inside." << endl;
        return EXIT_FAILURE;
    }
    ...
    pair<Mat, vector<bool>> homographyWithInliers = ransac<DMatch, Mat>(goodMatches, 4, 1.0,
